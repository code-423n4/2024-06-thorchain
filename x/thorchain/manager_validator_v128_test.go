...
type ValidatorMgrV128TestSuite struct{}

var _ = Suite(&ValidatorMgrV128TestSuite{})

func (vts *ValidatorMgrV128TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV128TestSuite) TestChurnWithChainHalt(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(60) // churn height
	mgr := NewDummyMgr()
	mgr.K = k
	networkMgr := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV128TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	validatorMgr := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr1 := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr2 := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV128TestSuite) TestRagnarokForMainnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	networkMgr := newValidatorMgrV128(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV128TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV128TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV128TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV128TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV128TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV128TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV128TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_EmptyNodeAccounts(c *C) {
	var vm ValidatorMgrV128
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_SingleNodeAccount(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_AllSameBondAddress(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_DifferentBondAddresses(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_SameBondSameAddress(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_ZeroAsgardSize(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_NegativeAsgardSize(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_AsgardSizeLargerThanLen(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_MultipleDuplicates(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV128TestSuite) TestSplitNext_MultipleDuplicatesOfDiffSizes(c *C) {
	var vm ValidatorMgrV128
	ctx, _ := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV128TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV128TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV128TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	validatorMgr := newValidatorMgrV128(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
...
func (vts *ValidatorMgrV128TestSuite) TestFindNextVaultNodeAccountsMax(c *C) {
	// test that we don't exceed the targetCount
	ctx, mgr := setupManagerForTest(c)
	validatorMgr := newValidatorMgrV128(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
	// create active nodes
	for i := 0; i < 12; i++ {
		na := GetRandomValidatorNode(NodeActive)
...
func (vts *ValidatorMgrV128TestSuite) TestFindNextVaultNodeAccountsBFT(c *C) {
...
func (vts *ValidatorMgrV128TestSuite) TestWeightedBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV128TestSuite) TestActiveNodeRequestToLeaveShouldBeStandby(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
