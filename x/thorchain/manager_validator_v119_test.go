...
type ValidatorMgrV119TestSuite struct{}

var _ = Suite(&ValidatorMgrV119TestSuite{})

func (vts *ValidatorMgrV119TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV119TestSuite) TestChurnWithChainHalt(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(60) // churn height
	mgr := NewDummyMgr()
	mgr.K = k
	networkMgr := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV119TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	validatorMgr := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr1 := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr2 := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV119TestSuite) TestRagnarokForMainnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	networkMgr := newValidatorMgrV119(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV119TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV119TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV119TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV119TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV119TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV119TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV119TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_EmptyNodeAccounts(c *C) {
	var vm ValidatorMgrV119
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_SingleNodeAccount(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_AllSameBondAddress(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_DifferentBondAddresses(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_SameBondSameAddress(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_ZeroAsgardSize(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_NegativeAsgardSize(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_AsgardSizeLargerThanLen(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_MultipleDuplicates(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV119TestSuite) TestSplitNext_MultipleDuplicatesOfDiffSizes(c *C) {
	var vm ValidatorMgrV119
	ctx, _ := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV119TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV119TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV119TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	validatorMgr := newValidatorMgrV119(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
...
func (vts *ValidatorMgrV119TestSuite) TestFindNextVaultNodeAccountsMax(c *C) {
	// test that we don't exceed the targetCount
	ctx, mgr := setupManagerForTest(c)
	validatorMgr := newValidatorMgrV119(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
	// create active nodes
	for i := 0; i < 12; i++ {
		na := GetRandomValidatorNode(NodeActive)
...
func (vts *ValidatorMgrV119TestSuite) TestFindNextVaultNodeAccountsBFT(c *C) {
...
func (vts *ValidatorMgrV119TestSuite) TestWeightedBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV119TestSuite) TestActiveNodeRequestToLeaveShouldBeStandby(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
