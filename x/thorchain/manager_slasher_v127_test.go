...
type SlashingV127Suite struct{}

var _ = Suite(&SlashingV127Suite{})

func (s *SlashingV127Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV127Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV127(k, NewDummyEventMgr())
...
func (s *SlashingV127Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV127(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV127Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV127Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV127Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV127Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV127Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV127Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV127(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV127Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV127(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV127Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV127(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV127Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
