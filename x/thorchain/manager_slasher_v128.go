...
// newSlasherV128 create a new instance of Slasher
func newSlasherV128(keeper keeper.Keeper, eventMgr EventManager) *SlasherV128 {
	return &SlasherV128{keeper: keeper, eventMgr: eventMgr}
}
...
func (s *SlasherV128) slashNotObserving(ctx cosmos.Context, txHash common.TxID, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV128) checkSignerAndSlash(ctx cosmos.Context, nodes NodeAccounts, blockHeight int64, signers []cosmos.AccAddress, constAccessor constants.ConstantValues) {
...
func (s *SlasherV128) LackSigning(ctx cosmos.Context, mgr Manager) error {
	...
	for i, toi := range txs.TxArray {
		...
		if toi.OutHash.IsEmpty() {
			...
			if s.needsNewVault(ctx, mgr, vault, signingTransPeriod, voter.FinalisedHeight, toi) {
...
func (s SlasherV128) slashAndUpdateNodeAccount(ctx cosmos.Context, na types.NodeAccount, coin common.Coin, vault types.Vault, totalBond, totalSlashAmountInRune cosmos.Uint) cosmos.Uint {
...
func (s *SlasherV128) updatePoolFromSlash(ctx cosmos.Context, pool types.Pool, stolenAsset common.Coin, runeCreditAmt cosmos.Uint, mgr Manager) {
...
func (s *SlasherV128) needsNewVault(ctx cosmos.Context, mgr Manager, vault Vault, signingTransPeriod, startHeight int64, toi TxOutItem) bool {
