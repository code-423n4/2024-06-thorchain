...
type SlashingV126Suite struct{}

var _ = Suite(&SlashingV126Suite{})

func (s *SlashingV126Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV126Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV126(k, NewDummyEventMgr())
...
func (s *SlashingV126Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV126(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV126Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV126Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV126Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV126Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV126Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV126Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV126(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV126Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV126(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV126Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV126(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV126Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
