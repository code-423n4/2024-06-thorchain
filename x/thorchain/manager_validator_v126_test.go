...
type ValidatorMgrV126TestSuite struct{}

var _ = Suite(&ValidatorMgrV126TestSuite{})

func (vts *ValidatorMgrV126TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV126TestSuite) TestChurnWithChainHalt(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(60) // churn height
	mgr := NewDummyMgr()
	mgr.K = k
	networkMgr := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV126TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	validatorMgr := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr1 := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	validatorMgr2 := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV126TestSuite) TestRagnarokForMainnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	networkMgr := newValidatorMgrV126(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV126TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV126TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV126TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV126TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV126TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV126TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
...
func (vts *ValidatorMgrV126TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_EmptyNodeAccounts(c *C) {
	var vm ValidatorMgrV126
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_SingleNodeAccount(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_AllSameBondAddress(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_DifferentBondAddresses(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_SameBondSameAddress(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_ZeroAsgardSize(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_NegativeAsgardSize(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_AsgardSizeLargerThanLen(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_MultipleDuplicates(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (s *ValidatorMgrV126TestSuite) TestSplitNext_MultipleDuplicatesOfDiffSizes(c *C) {
	var vm ValidatorMgrV126
	ctx, _ := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV126TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV126TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV126TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	validatorMgr := newValidatorMgrV126(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
...
func (vts *ValidatorMgrV126TestSuite) TestFindNextVaultNodeAccountsMax(c *C) {
	// test that we don't exceed the targetCount
	ctx, mgr := setupManagerForTest(c)
	validatorMgr := newValidatorMgrV126(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(validatorMgr, NotNil)
	// create active nodes
	for i := 0; i < 12; i++ {
		na := GetRandomValidatorNode(NodeActive)
...
func (vts *ValidatorMgrV126TestSuite) TestFindNextVaultNodeAccountsBFT(c *C) {
...
func (vts *ValidatorMgrV126TestSuite) TestWeightedBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV126TestSuite) TestActiveNodeRequestToLeaveShouldBeStandby(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
