...
type SlashingV125Suite struct{}

var _ = Suite(&SlashingV125Suite{})

func (s *SlashingV125Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV125Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV125(k, NewDummyEventMgr())
...
func (s *SlashingV125Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV125(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV125Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV125Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV125Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV125Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV125Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV125Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV125(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV125Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV125(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV125Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV125(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV125Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	...
	c.Check(slasher.needsNewVault(ctx, mgr, vault, 300, 1, inhash, pk), Equals, false)
