...
type SlashingV128Suite struct{}

var _ = Suite(&SlashingV128Suite{})

func (s *SlashingV128Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV128Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV128(k, NewDummyEventMgr())
...
func (s *SlashingV128Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV128(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV128Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV128Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV128Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV128Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV128Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV128Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV128(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV128Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV128(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV128Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV128(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV128Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
