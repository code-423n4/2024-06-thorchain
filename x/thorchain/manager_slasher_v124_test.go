...
type SlashingV124Suite struct{}

var _ = Suite(&SlashingV124Suite{})

func (s *SlashingV124Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV124Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV124(k, NewDummyEventMgr())
...
func (s *SlashingV124Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV124(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV124Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV124Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV124Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV124Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
	...
	err = slasher.HandleDoubleSign(ctx, pk.Address(), 0, constAccessor, []nodeAddressValidatorAddressPairV124{pair})
	c.Assert(err, IsNil)
...
func (s *SlashingV124Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV124Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV124(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV124Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV124(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV124Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV124(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV124Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
