...
type SlashingV116Suite struct{}

var _ = Suite(&SlashingV116Suite{})

func (s *SlashingV116Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV116Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV116(k, NewDummyEventMgr())
...
func (s *SlashingV116Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV116(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV116Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV116Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV116Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV116Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV116Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV116Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV116(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV116Suite) TestSlashAndUpdateNodeAccount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV116(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV116Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV116(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV116Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV116(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV116Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
