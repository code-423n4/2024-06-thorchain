...
type SlashingV129Suite struct{}

var _ = Suite(&SlashingV129Suite{})

func (s *SlashingV129Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV129Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV129(k, NewDummyEventMgr())
...
func (s *SlashingV129Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV129(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV129Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV129Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV129Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV129Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV129Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV129Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV129(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV129Suite) TestUpdatePoolFromSlash(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV129(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV129Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV129(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV129Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
