...
type GasManagerTestSuiteV131 struct{}

var _ = Suite(&GasManagerTestSuiteV131{})

func (GasManagerTestSuiteV131) TestGasManagerVCUR(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.K
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV131(constAccessor, k)
...
func (GasManagerTestSuiteV131) TestGetFee(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.Keeper()
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV131(constAccessor, k)
...
func (GasManagerTestSuiteV131) TestDifferentValidations(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.Keeper()
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV131(constAccessor, k)
...
func (GasManagerTestSuiteV131) TestGetMaxGas(c *C) {
...
func (GasManagerTestSuiteV131) TestOutboundFeeMultiplier(c *C) {
	ctx, k := setupKeeperForTest(c)
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV131(constAccessor, k)
